...
func main() {
	...
	ui, err := circuit.From(&cfg,
		...
		circuit.WithActions(
			...
			}.Describe("Restart the application service (requires confirmation)").Confirm().WithTimeout(30*time.Second),
			...
			}.Describe("Hot reload configuration without downtime").WithTimeout(10*time.Second),
			...
			}.Describe("Verify service health and connectivity").WithTimeout(5*time.Second),
			...
			}.Describe("Clear all cached data").WithTimeout(15*time.Second),
